        ...
        gpointer data )
{
    ...
    for(gint i = 0; i < n_files; i++)
    {
        ...
        gchar *uri = g_file_get_path(((GFile **)files)[i]);
            ...
            gpointer data )
{
    ...
    for (gint i = 0; i < n_files; i++)
    {
        files[i] =  g_application_command_line_create_file_for_arg(cli, argv[i+1]);
    }
    ...
    if (n_files > 0)
    {
        g_application_open(gapp, files, n_files, app->enqueue ? "enqueue" : "");
    }
