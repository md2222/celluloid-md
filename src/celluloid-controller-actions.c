...
static void
playlist_select_next_handler(GSimpleAction *action, GVariant *param, gpointer data);
...
playlist_select_next_handler(GSimpleAction *action, GVariant *param, gpointer data)
{
    //printf("playlist_select_next_handler:  \n");
    ...
    {
        CelluloidPlaylistWidget *playlist = celluloid_main_window_get_playlist(wnd);

        celluloid_playlist_widget_select_next(playlist, 1);
    }
}


static void
playlist_select_prev_handler(GSimpleAction *action, GVariant *param, gpointer data)
{
    //printf("playlist_select_next_handler:  \n");
...
static void
playlist_select_play_handler(GSimpleAction *action, GVariant *param, gpointer data)
{
    //printf("playlist_select_play_handler:  \n");
    ...
    if (celluloid_main_window_get_playlist_visible(wnd))
    {
        ...
        celluloid_playlist_widget_play_selected(playlist);
    }
}


static void
key_forward_handler(GSimpleAction *action, GVariant *param, gpointer data)
{
    CelluloidController *controller = data;

    celluloid_model_forward(controller->model);
}    


static void
key_rewind_handler(GSimpleAction *action, GVariant *param, gpointer data)
{
    CelluloidController *controller = data;

    celluloid_model_rewind(controller->model);
}    


static void
key_forward_min_handler(GSimpleAction *action, GVariant *param, gpointer data)
{
    CelluloidController *controller = data;

    celluloid_model_seek_sec(controller->model, "60");
}    


static void
key_rewind_min_handler(GSimpleAction *action, GVariant *param, gpointer data)
{
    CelluloidController *controller = data;
...
static void
clear_playlist_handler(  GSimpleAction *action,
                    GVariant *param,
                    gpointer data )
{
    CelluloidView *view = celluloid_controller_get_view(data);
    ...
    if(celluloid_main_window_get_playlist_visible(wnd))
    {
        ...
        celluloid_playlist_widget_clear(playlist);
    }
}
...
celluloid_controller_action_register_actions(CelluloidController *controller)
{
    const GActionEntry entries[]
        = { {.name = "open",
            ...
            {.name = "toggle-playlist", .activate = toggle_playlist_handler},
            {.name = "playlist-select-next", .activate = playlist_select_next_handler},
            {.name = "playlist-select-prev", .activate = playlist_select_prev_handler},
            {.name = "playlist-select-play", .activate = playlist_select_play_handler},
            {.name = "key-forward", .activate = key_forward_handler},
            {.name = "key-rewind", .activate = key_rewind_handler},
            {.name = "key-forward-min", .activate = key_forward_min_handler},
            {.name = "key-rewind-min", .activate = key_rewind_min_handler},
            {.name = "save-playlist",
            ...
            {.name = "remove-selected-playlist-item", .activate = remove_selected_playlist_item_handler},
            {.name = "clear-playlist", .activate = clear_playlist_handler},
