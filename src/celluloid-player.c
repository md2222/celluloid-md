...
observe_properties(CelluloidMpv *mpv)
{
    ...
    celluloid_mpv_observe_property(mpv, 0, "path", MPV_FORMAT_STRING);
...
apply_default_options(CelluloidMpv *mpv)
{
    ...
    options[] = {   {"vo", "opengl,vdpau,vaapi,xv,x11,libmpv,"},
            ...
            {"screenshot-template", "video-shot%n"},
            {NULL, NULL} };
...
parse_playlist_entry(mpv_node_list *node)
{
    ...
    title = get_name_from_path(filename);  //MD
...
void
celluloid_player_clear_playlist(CelluloidPlayer *player, gint64 position)
{
    CelluloidMpv *mpv = CELLULOID_MPV(player);
    gboolean idle_active =  celluloid_mpv_get_property_flag(mpv, "idle-active");
    gint64 playlist_count = 0;

    celluloid_mpv_get_property(mpv, "playlist-count", MPV_FORMAT_INT64, &playlist_count);
    ...
    if (idle_active || playlist_count == 1)
    {
        g_ptr_array_remove_index(get_private(player)->playlist, (guint)position );
        g_object_notify(G_OBJECT(player), "playlist");
    }

    if (!idle_active)
    {
        const gchar *cmd[] = {"playlist_clear", NULL, NULL};
