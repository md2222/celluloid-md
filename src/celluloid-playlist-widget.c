...
constructed(GObject *object)
{
    ...
    //self->store = gtk_list_store_new( 4, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_INT, G_TYPE_STRING);
    ...
    g_signal_connect(   self->scrolled_window,
                "key-press-event",
    ...
    GtkStyleContext* listStyle = gtk_widget_get_style_context (self->tree_view);
    GdkRGBA bgColor;
    gtk_style_context_get_background_color (listStyle, GTK_STATE_FLAG_NORMAL, &bgColor);
    double bright = 0.2 * bgColor.red + 0.7 * bgColor.green + 0.1 * bgColor.blue;
    //gchar* bgColorStr = gdk_rgba_to_string (&bgColor);
    //printf("celluloid_playlist_widget_init:  %s  bright=%f\n", bgColorStr, bright);
    self->currRowColor = "#306100";
    if (bright > 0.5) self->currRowColor = "#96fc90";
    ...
    gtk_container_add(GTK_CONTAINER(self->overlay), self->scrolled_window);
    gtk_box_pack_end(GTK_BOX(self), self->overlay, TRUE, TRUE, 0);
        ...
        btn_event->button == 3 )
    {
        const CelluloidMenuEntry entries[]
            = { CELLULOID_MENU_SEPARATOR,
                ...
                CELLULOID_MENU_SEPARATOR,
                CELLULOID_MENU_ITEM(_("Clear"), "win.clear-playlist"),
                CELLULOID_MENU_END };
...
celluloid_playlist_widget_init(CelluloidPlaylistWidget *wgt)
{
    ...
    wgt->title_column = gtk_tree_view_column_new_with_attributes
            (   _("Playlist"),
                wgt->title_renderer,
                "text", PLAYLIST_NAME_COLUMN,
//                "weight", PLAYLIST_WEIGHT_COLUMN,
                "cell-background", PLAYLIST_BG_COLUMN,
                NULL );
...
void
celluloid_playlist_widget_set_indicator_pos( CelluloidPlaylistWidget *wgt,  gint pos )
{
    //printf("celluloid_playlist_widget_set_indicator_pos:\n");
/*    
    GtkStyleContext* ctx = gtk_widget_get_style_context (wgt->tree_view);
    //GdkRGBA fgColor;
    //gtk_style_context_get_color (ctx, GTK_STATE_FLAG_NORMAL, &fgColor);
    GdkRGBA bgColor;
    gtk_style_context_get_background_color (ctx, GTK_STATE_FLAG_NORMAL, &bgColor);
    
    double bright = 0.2 * bgColor.red + 0.7 * bgColor.green + 0.1 * bgColor.blue;
    
    const gchar* bgStr = "#397300";
    if (bright > 0.4) bgStr = "#7cf800";
*/    
    /*gchar* fgColorStr = gdk_rgba_to_string (&fgColor);
    gchar* bgColorStr = gdk_rgba_to_string (&bgColor);
    printf("fgColor=%s  %s  %f\n", fgColorStr, bgColorStr, bright);
    g_free(fgColorStr);
    g_free(bgColorStr);*/
    ...
    while (rc)
    {
        //const PangoWeight weight = (pos-- == 0) ? PANGO_WEIGHT_BOLD : PANGO_WEIGHT_NORMAL;

        //gtk_list_store_set(wgt->store, &iter, PLAYLIST_WEIGHT_COLUMN, weight, PLAYLIST_BG_COLUMN, "#ff0000", -1 );

        //GdkColor color;
        //gdk_color_parse("#ff0000", &color);
        
        const gchar* bg = (pos-- == 0) ? wgt->currRowColor : NULL;

        //gtk_list_store_set(wgt->store, &iter, PLAYLIST_WEIGHT_COLUMN, weight, PLAYLIST_BG_COLUMN, bg, -1);
        gtk_list_store_set(wgt->store, &iter, PLAYLIST_BG_COLUMN, bg, -1 );

        /*if (pos-- == 0)
            gtk_list_store_set(wgt->store, &iter, PLAYLIST_BG_COLUMN, "#202020", -1 );
        else
            gtk_list_store_set(wgt->store, &iter, PLAYLIST_BG_COLUMN, "#404040", -1 );*/
...
void
celluloid_playlist_widget_clear(CelluloidPlaylistWidget *wgt)
{
    gtk_list_store_clear (wgt->store);
}


static gboolean scrollTreeview(CelluloidPlaylistWidget *wgt)
{
    //printf("scrollTreeview:\n");
    GtkTreePath *path = NULL;
    ...
    gtk_tree_view_scroll_to_cell(wgt->tree_view, path, NULL, FALSE, 0.0, 0.0);
...
void
celluloid_playlist_widget_select_next( CelluloidPlaylistWidget *wgt, gint step )
{
    //printf("celluloid_playlist_widget_play_next:\n");
    ...
    if (path)
    {
        ...
        if ( gtk_tree_model_get_iter(model, &iter, path) )
        {
            if ( step > 0 && !gtk_tree_model_iter_next (model,  &iter) )  return;
            else if ( step < 0 && !gtk_tree_model_iter_previous (model,  &iter) )  return;
            
            if (step)
            {
                GtkTreePath *path = NULL;
                path = gtk_tree_model_get_path(model, &iter);   
                gtk_tree_view_set_cursor(wgt->tree_view, path, NULL, FALSE);
                gtk_tree_view_scroll_to_cell(wgt->tree_view, path, NULL, FALSE, 0.0, 0.0);
                //g_timeout_add(500, scrollTreeview, wgt); //--
...
void
celluloid_playlist_widget_play_selected(CelluloidPlaylistWidget *wgt)
{
    GtkTreePath *path = NULL;
    ...
    if(path)
    {
        ...
        g_signal_emit_by_name(wgt, "row-activated", index);
    }
}
                        ...
                        GPtrArray* playlist )
{
    ...
    for(guint i = 0; i < playlist->len; i++)
    {
        ...
        if (!g_strcmp0(title, "''"))  title = NULL;  //MD
