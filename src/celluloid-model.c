        ...
        GParamSpec *pspec )
{
    ...
    switch(property_id)
    {
        ...
        self->filename = get_name_from_path(g_value_dup_string(value));
        break;
...
celluloid_model_class_init(CelluloidModelClass *klass)
{
    ...
    mpv_props[] = { {"aid", PROP_AID, G_TYPE_STRING},
            ...
            {"path", PROP_FILENAME, G_TYPE_STRING},
...
{
    //printf("celluloid_model_key_down:  keystr=%s\n", keystr);
...
{
    const gchar *cmd[] = {"seek", "15", "exact", NULL};
...
void
celluloid_model_seek_sec(CelluloidModel *model, gchar* sec)
{
    const gchar *cmd[] = {"seek", sec, "exact", NULL};
...
{
    celluloid_player_remove_playlist_entry(CELLULOID_PLAYER(model), position);
}
...
static gint pathCmpFunc (gconstpointer a, gconstpointer b)
{
    const gchar *p1 = *((gchar **)a);
    const gchar *p2 = *((gchar **)b);
    
    //gint res = g_strncasecmp(p1, p2, g_strlen())
    //return g_ascii_strcasecmp (p1, p2);
    return g_strcasecmp(p1, p2);
}
...
celluloid_model_load_file(  CelluloidModel *model,
                ...
                gboolean append )
{
    //printf("celluloid_model_load_file:    %s\n", uri);
    ...
    if ( stat(uri, &s) == 0 && s.st_mode & S_IFDIR )
    {
        //printf("celluloid_model_load_file:    uri is dir\n");
        isDir = true;
    }
    ...
    if (isDir)
    {
        ...
        if (d) 
        {
            ...
            {
                if ( !g_strcmp0(dir->d_name, ".") || !g_strcmp0(dir->d_name, "..") )  continue;
                
                gchar* type = g_content_type_guess (dir->d_name, NULL, 0, NULL);
                //printf("type=%s\n", type);
                
                if (type)
                    for (int i = 0; mime_types[i]; i++)
                        if ( !g_strcmp0(type, mime_types[i]) )
                            g_ptr_array_add(files, dir->d_name);
                            
                g_free(type);
                //printf("celluloid_model_load_file:    file in dir = %s\n", dir->d_name);
            }
            ...
            g_ptr_array_sort(files, pathCmpFunc);
            
            for (guint i = 0; i < files->len; i++)
            {
                gchar* path = g_strconcat(uri, "/", g_ptr_array_index(files, i), NULL);   
                //printf("celluloid_model_load_file:    file in array = %s\n", path);
                celluloid_mpv_load_file(CELLULOID_MPV(model), path, TRUE);
            }
    ...
    if (!isDir)
    {
        //gchar* type = g_content_type_guess (uri, NULL, 0, NULL);
