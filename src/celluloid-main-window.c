                ...
                gpointer data )
{
    ...
    g_signal_handlers_disconnect_by_func(widget, resize_video_area_finalize, data);

    gdk_monitor_get_geometry(monitor, &monitor_geom);

    /* Adjust resize offset */
    if((width != target_width || height != target_height) && (target_width < monitor_geom.width 
        && target_height < monitor_geom.height ) && !gtk_window_is_maximized(GTK_WINDOW(wnd))
        && !priv->fullscreen)
    {
        ...
        priv->resize_tag = g_idle_add_full( G_PRIORITY_HIGH_IDLE, resize_to_target, wnd, NULL );
    }
}
...
celluloid_main_window_init(CelluloidMainWindow *wnd)
{
    ...
    //gtk_paned_set_position (GTK_PANED(priv->vid_area_paned), 500);   //-- 
...
static gboolean afterLoad(CelluloidMainWindow *wnd)
{
    /*CelluloidMainWindowPrivate *priv = get_private(wnd);
    ...
    printf("afterLoad:  width=%d\n", width);
    gint handle_pos = width - (priv->playlist_visible ? priv->playlist_width : 0);

    gtk_paned_set_position(GTK_PANED(priv->vid_area_paned), handle_pos);*/
...
celluloid_main_window_load_state(CelluloidMainWindow *wnd)
{
    if(!gtk_widget_get_realized(GTK_WIDGET(wnd)))
    {
        ...
        priv->playlist_width = g_settings_get_int(settings, "playlist-width");
        priv->playlist_visible = g_settings_get_boolean(settings, "show-playlist");
        controls_visible = g_settings_get_boolean(settings, "show-controls");
        ...
        //printf("celluloid_main_window_load_state:  width=%d\n", width);
        gtk_paned_set_position(GTK_PANED(priv->vid_area_paned), handle_pos);
        //g_idle_add(afterLoad, wnd); 

        if (maximized)
        {
            //gtk_window_maximize(GTK_WINDOW(wnd));
            g_timeout_add(1000, afterLoad, wnd); 
        }
